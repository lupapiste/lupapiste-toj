...
Install MongoDB >= 5.0 and create a database called `toj`. Your MongoDB should not enforce authentication.
...
When connecting with SSL/TLS encryption, add `ssl=true` to the end of the connection string, e.g.:
...
The classic figwheel seems to work poorly with at least Apple Silicon Macs though, so automatic reloading is currently
disabled, and you need to reload the page manually after changes.
