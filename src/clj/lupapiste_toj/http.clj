...
(def Config {:port s/Int
             ...
             :thread s/Int
             (s/optional-key :cookie-attrs) {s/Keyword s/Any}
             (s/optional-key :cookie-name) s/Str
             (s/optional-key :csp) {s/Keyword s/Any}})

(defn start-http-server [component {:keys [port mode] :as config} db build-info]
