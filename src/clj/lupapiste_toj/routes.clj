...
(defn create-routes [{:keys [mode db build-info session-key-path autologin-check-url lupapiste-host csp] :as config}]
  (let [store (rs/rekeyable session-key-path)]
    (-> (routes (-> (routes index-redirect
                            (index-route build-info))
                    (wrap-routes #(st/wrap-session-timeout % (redirect lupapiste-host))))
                (rekey-route store)
                (-> (context routing/root []
                    ...
                    (wrap-routes st/wrap-session-timeout)))
        (ru/optional-middleware ru/wrap-request-logging (= mode :prod))
        (auto/wrap-sso-autologin autologin-check-url)
        (wrap-session (merge {:store store}
                             (select-keys config [:cookie-attrs :cookie-name])))
        (ru/wrap-csp-header csp)
